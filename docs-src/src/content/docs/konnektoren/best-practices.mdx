...
Ohne gesetzte Regel wird ein Konnektor nicht automatisch gesperrt. Ein frisch angelegter Konnektor ist damit für jede Person sichtbar, die überhaupt MCP benutzen darf.

Das ist bewusst so, weil ein Konfigurationsfehler nicht den ganzen Chat lahmlegen soll. Für Sie bedeutet es aber, dass der Zeitraum zwischen „Konnektor angelegt" und „Rechte gesetzt" ein offenes Fenster ist. Bei einer ERP-Anbindung sind das in diesem Fenster sämtliche Kunden, Belege und Personaldaten, die das Dienstkonto sehen kann.
...
Eine Anbindung an Ihre Dateiablage funktioniert typischerweise so, siehe [Muster B](/docs/konnektoren/muster). Der Preis dafür ist, dass jede Person sich einmal verbinden muss, und dass für einen solchen Konnektor keine Freigaben pro Rolle oder Gruppe gelten. Das ist der richtige Tausch: Eine feinere Grenze im Zielsystem ist mehr wert als eine gröbere Grenze davor.
...
Tragen Sie Zugangsdaten niemals im Klartext in die Konfiguration ein, sondern ausschliesslich als Referenz auf eine Umgebungsvariable, also in der Form `${MEIN_SCHLUESSEL}`. Den Wert selbst hinterlegen Sie im Panel unter **Einstellungen → API-Schlüssel**.
...
- [ ] Der Verbindungstest ist grün, und die Instanz wurde neu gestartet.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Die Rechtekaskade und wo Sie die Freigaben pflegen."
  />
  <LinkCard
