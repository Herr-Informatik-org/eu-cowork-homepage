(() => {
  ...
  function createParticle() {
    return {
      ...
      radius: .6 + Math.random() * 2.1,
      vx: -.13 + Math.random() * .26, vy: -.13 - Math.random() * .38,
      phase: Math.random() * Math.PI * 2, petal: Math.random() > .84,
      opacity: .15 + Math.random() * .48
    };
  }
  ...
  function drawParticle(particle, time, step) {
    ...
    particle.x += (particle.vx + Math.sin(time * .0003 + particle.phase) * .13) * step;
    particle.y += particle.vy * step;
    if (particle.y < -15) { particle.y = height + 15; particle.x = Math.random() * width; }
    if (particle.x < -20) particle.x = width + 20;
    if (particle.x > width + 20) particle.x = -20;
    context.save();
    context.translate(particle.x, particle.y);
    context.globalAlpha = particle.opacity * (.75 + Math.sin(time * .001 + particle.phase) * .25);
    context.fillStyle = particle.petal ? '#d77a65' : '#e9d9a0';
    context.beginPath();
    if (particle.petal) {
      context.rotate(time * .00015 + particle.phase);
      context.ellipse(0, 0, particle.radius * 2.3, particle.radius * .75, 0, 0, Math.PI * 2);
    } else {
  ...
  function tick(time) {
    ...
    const targetX = pointer.active ? (pointer.x / width - .5) : 0;
    const targetY = pointer.active ? (pointer.y / height - .5) : 0;
    pointer.smoothX += (targetX - pointer.smoothX) * .055 * step;
    pointer.smoothY += (targetY - pointer.smoothY) * .055 * step;
  ...
  function applyMotionState() {
    ...
    motionButton.title = reducedMotion.matches
      ? translate('systemMotion', 'Reduzierte Bewegung ist in deinen Systemeinstellungen aktiviert.')
  ...
  const teaResponses = [
    ['teaReply1', 'Eine ausgezeichnete Wahl. Frisch aufgebrüht, selbstverständlich. Die letzte Tasse hat nur ganz kurz geleuchtet.'],
    ['teaReply2', 'Noch eine Tasse? Du hast Geschmack. Der Tee ist bereits produktionsreif. Die KI bekommt noch den letzten Schliff.'],
    ['teaReply3', 'Langsam glaube ich, du bist gar nicht wegen der KI hier. Keine Sorge. Das bleibt unter uns.'],
    ['teaReply4', 'Mein Vorrat ist unerschöpflich. Im Gegensatz zum Geduldsfaden meiner Entwickler. Noch ein Schluck?']
  ];
  ...
  teaButton.addEventListener('click', () => {
    ...
    closing.textContent = translate('teaClosing', 'Lehn dich zurück. Ich kümmere mich um den Rest.');
    speech.replaceChildren(message, closing);
    document.querySelector('#tea-label').textContent = translate('moreTea', 'Noch eine Tasse');
    if (!paused) {
      const bounds = teaButton.getBoundingClientRect();
      bursts = bursts.concat(Array.from({ length: 28 }, () => {
        const angle = Math.random() * Math.PI * 2;
        const speed = .8 + Math.random() * 2;
        return { x: bounds.x + bounds.width / 2, y: bounds.y + bounds.height / 2,
          vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed - 1,
          radius: .8 + Math.random() * 1.8, life: 1,
          color: Math.random() > .6 ? '#d77a65' : '#e9d9a0' };
      })).slice(-112);
    }
  });

  motionButton.addEventListener('click', () => { manualPause = !manualPause; applyMotionState(); });
